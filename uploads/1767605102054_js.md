...
* 很少使用
symbol的特点是唯一性，每个通过Symbol创建的值都是独一无二的
```
const sym1 = Symbol();
const sym2 = Symbol();
...
console.log(symWithDesc.description); // "这是一个描述"（ES2019 新增的 description 属性）
console.log(String(symWithDesc)); // "Symbol(这是一个描述)"
```

注意：即使描述相同，两个Symbol也不相等
```
...
* 不同浏览器对打印 son 的 `__proto__` 属性时显示不同
  * Edge Google 打印 console.dir(son) 显示 `[[Prototype]]`
...
o1.address.city = '河北';    //这里修o1的属性后，o2的属性也会被改为'河北'
```
