...
1. **已知** 系统已就绪，用户已登录，**当** 用户上传一个 PDF 文档并填写标题、标签、描述，**则** 系统应在合理时间内完成入库，显示成功消息，用户可在资源列表中查看该资源及其元数据
...
1. **已知** 系统中已有相关学习资源，用户已登录，**当** 用户在问答界面输入问题"什么是 RAG？"，**则** 系统应在合理时间内返回答案，答案文本下方显示引用证据卡片（包含资源标题、页码/范围、相关文本摘录）
...
2. **已知** 用户点击了某个推荐资源，**当** 系统记录该行为，**则** 后续推荐应反映这一兴趣（如增加该资源相关主题的权重），用户画像应更新
...
**优先级理由**：用户账户和行为记录是所有个性化功能的前提。行为数据也是评估系统效果（离线评测分析）的基础。此功能在 MVP 阶段必须实现，但实现方式可简化（如仅记录查询和点击行为）。
...
- 当同时上传大量资源（如批量上传 100+ 文件）时，系统应提供进度反馈，并在处理完成后通知用户
...
- **FR-015**：系统必须在检索证据不足时（如未找到相关切片或相似度分数过低）给出明确说明，并提供降级返回（如显示检索到的候选资源列表或证据摘要）
- **FR-016**：系统必须构建轻量用户画像：基于用户最近 N 次查询生成兴趣向量（采用滑动平均或加权平均方法，N 可配置）
- **FR-017（可选增强）**：系统可支持在画像计算中加入点击/收藏行为权重（可配置开关与权重值）；MVP 可先仅基于查询行为构建画像。
...
- **FR-020**：系统必须为推荐理由提供证据引用（格式同问答引用规范：包含 resourceId、chunkId、chunkIndex、snippet）
**引用（Citations）结构规范（必须）**  
系统对外返回的 citations 应使用稳定 JSON 结构，至少包含：
- resourceId, resourceTitle
- chunkId, chunkIndex
- location：PDF 用 pageStart/pageEnd；Markdown 用 heading/section 或等价定位信息
- snippet：用于支撑结论的原文摘录（长度可截断）
- **FR-021**：系统必须支持小规模离线测试集管理：人工设计查询问题并标注相关资源列表（每个查询对应 1-N 个相关资源）
...
- **查询行为 (QueryBehavior)**：用户查询记录，包含属性：行为 ID、用户 ID（外键）、查询文本、时间戳
- **交互行为 (InteractionBehavior)**：用户点击/收藏记录，包含属性：行为 ID、用户 ID（外键）、资源 ID（外键）、行为类型（点击/收藏）、时间戳、权重（可选，用于画像计算）
...
- **SC-002（目标）**：问答请求可在可接受时间内返回结果，并记录总耗时与分段耗时；当存在相关资源时，返回结果包含至少 1 条可验证引用（citations）。
